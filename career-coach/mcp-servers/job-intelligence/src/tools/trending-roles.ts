...
export async function handleTrendingRoles(input: TrendingRolesInput): Promise<string> {
  ...
  for (let i = 0; i < roles.length; i++) {
    ...
    if (role.demand_level === "high") demandIndicator = "HIGH";
    else if (role.demand_level === "medium") demandIndicator = "MEDIUM";
    else if (role.demand_level === "low") demandIndicator = "LOW";
 ...
 */
function parseSalaryEstimate(text: string): number {
  if (!text) return 0;
  ...
  if (singleMatch) {
    ...
    if (singleMatch[2]?.toLowerCase() === "k") return num * 1000;
    if (singleMatch[2]?.toLowerCase() === "m") return num * 1_000_000;
    return num;
  }
