...
export async function handleInterviewExperiences(input) {
    ...
    const offers = experiences.map((e) => e.offer).filter((o) => o !== null);
    ...
    for (let i = 0; i < experiences.length; i++) {
        ...
        const meta = [];
        if (exp.difficulty)
            meta.push(`**Difficulty:** ${exp.difficulty}`);
        if (exp.experience)
            meta.push(`**Outcome:** ${exp.experience}`);
        if (exp.offer !== null)
            meta.push(`**Offer:** ${exp.offer ? "Yes" : "No"}`);
        if (exp.date)
            meta.push(`**Date:** ${exp.date}`);
        if (meta.length > 0) {
