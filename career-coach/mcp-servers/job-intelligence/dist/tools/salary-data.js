...
export async function handleSalaryData(input) {
    ...
    for (let i = 0; i < displaySalaries.length; i++) {
        ...
        lines.push(`| ${i + 1} | ${s.role} | ${s.company || "N/A"} | ${formatDollar(s.base)} | ${formatDollar(s.equity)} | ${formatDollar(s.bonus)} | ${formatDollar(s.total_comp)} | ${s.level || "N/A"} | ${s.source} |`);
    }
    lines.push("");
    // Summary statistics if we have enough data
    const validTotals = allSalaries.map((s) => s.total_comp).filter((t) => t !== null);
    const validBases = allSalaries.map((s) => s.base).filter((b) => b !== null);
...
//# sourceMappingURL=salary-data.js.map