 ...
 */
function sortByRelevance(listings, query) {
...
export async function handleSearchJobs(input) {
    ...
    if (salary_min) {
        finalListings = finalListings.filter((listing) => {
            if (!listing.salary)
                return true; // Keep listings without salary info
    ...
    if (experience_level)
        lines.push(`**Experience Level:** ${experience_level}`);
    if (salary_min)
        lines.push(`**Minimum Salary:** $${salary_min.toLocaleString()}`);
    ...
    if (detailCount > 0) {
        ...
        for (let i = 0; i < detailCount; i++) {
            ...
            if (job.url)
                lines.push(`[View listing](${job.url})`);
            lines.push("");
        }
    }
